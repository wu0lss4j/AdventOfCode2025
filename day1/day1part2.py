...
def main():
    ...
    amount: int = 50
    position_prev: int = 0
    position: int = position_prev + direction * amount
    counter: int = 0
    passes: int = 0
    ...
    for row in raw_data.splitlines():
        ...
        position_prev = position # specifically for part 2 of Day 1
        position = turn(position, direction, amount)
        #print(f"{position}")

        passes += (max(position_prev, position)-1)//100 - (min(position_prev, position)//100)
    ...
    print(f"Number of dial passes through 0: {passes}")
    print(f"Total: {counter + passes}")
