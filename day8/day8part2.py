...
"""
I believe there would be an even faster way to get this, as boxes get added into circuits as well as when circuits get merged, you can trim all the distances between boxes in the same circuit. The search space thus reduces. However I am not sure if trimming the search space is faster then just powering through it once from the start state of an ordered list.
"""
...
last_item1: int | None = None
last_item2: int | None = None
...
for i, item in enumerate(dist3D_matrix_list):
    ...
    if connections_made > 10 and len(circuits) == 1:
        break
    ...
    last_item1 = item[1]
    last_item2 = item[2]
    print(f"\n  {i=:_<6}  Checking: {item[1]} to {item[2]} | {boxes_available=}")
...
print(f"{last_item1} and {last_item2}")
product = boxes[last_item1][0] * boxes[last_item2][0]
print(f"{boxes[last_item1][0]:.0f} * {boxes[last_item2][0]:.0f} = {product:,.0f}")
print(f"product of X position distance (two last connected boxes): {product:.0f}")
