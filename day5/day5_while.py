...
# MERGE from the finish to start of list using while loop
...
while not done:
    iterations += 1
    num_merged_classes_before = len(classes_u)
    for i in range(len(classes_u) - 1, 0, -1):
        # print(f"{classes_u=}")
        print(f"{i=} {classes_u[i]} vs {classes_u[i - 1]}")

        if classes_u[i - 1][1] >= classes_u[i][0]:
            if classes_u[i - 1][1] >= classes_u[i][1]:
                # print("contains")
                classes_u.pop(i)
            else:
                # print("overlaps")
                classes_u[i - 1][1] = classes_u[i][1]
                classes_u.pop(i)
        # print(f"{classes_u=}")
        num_merged_classes_after = len(classes_u)

    # print(f".....merged {(classes_u)=}")

    if (
        num_merged_classes_before == num_merged_classes_after
        or iterations == iterations_max
    ):
        done = True
print(f"  while not True {iterations=}")
# classes_m = classes_u.copy()

# print(f".....merged {classes_m=}")
